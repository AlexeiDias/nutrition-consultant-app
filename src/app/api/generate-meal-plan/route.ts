...
async function generateBatch(
  ...
  const calorieInstruction = calorieTarget > 0
    ? `The client's daily calorie target is ${calorieTarget} kcal. Distribute across 5 meals approximately: Breakfast 25% (~${Math.round(calorieTarget * 0.25)} kcal), Snack 10% (~${Math.round(calorieTarget * 0.10)} kcal), Lunch 30% (~${Math.round(calorieTarget * 0.30)} kcal), Afternoon Snack 10% (~${Math.round(calorieTarget * 0.10)} kcal), Dinner 25% (~${Math.round(calorieTarget * 0.25)} kcal).`
    : `Use appropriate calorie amounts for the goal: "${programGoal}".`;
...
Now generate ${numberOfDays} days starting from day ${startDayNumber}. Vary meals each day. Return ONLY the JSON array.`;
...
export async function POST(req: NextRequest) {
  try {
    const { programGoal, numberOfDays, startDate, tdee, calorieTarget, selectedGoals } =
      await req.json();
    ...
    console.log(`Generating ${cappedDays}-day plan. Goal: ${programGoal}. Calorie target: ${calorieTarget || 'not set'}`);
